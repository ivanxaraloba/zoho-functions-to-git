...
import { ArrowUpFromLine, ArrowUpRightFromSquare, ChevronsUpDown, CircleFadingArrowUp, Frown, LucideAArrowDown, Parentheses, RefreshCcw, SearchX, SquareArrowOutUpRight, TriangleAlert, X } from 'lucide-react';
...
import { AppTabContent, AppTabContentBody, AppTabContentHead, AppTabContentMissing, AppTabDescription, AppTabHeader } from '@/components/layout/app-tab';
...
export default function TabFunctions({ username }: { username: string }) {
  ...
  const queryCRM = useQuery({
    queryKey: ['project_crm_functions', username],
    queryFn: async () => await getCRM(project?.id),
    enabled: !!username,
  });

  console.log(queryCRM.data);
  ...
  const mutationRefresh = useMutation({
    mutationFn: async () => {
      ...
      const { data: functions, error: errorGetFunctions } = await crmGetFunctions(project.domain, project.crm?.config);
      ...
      const filteredFunctions = lastSync
        ? functions.filter(({ updatedTime, createdTime }) => updatedTime >= lastSync || createdTime >= lastSync)
        : functions;
      ...
      const functionsWithCode = (
        await Promise.all(
          filteredFunctions.map(async (functionInfo, index) => {
            const { data, error } = await crmGetFunction(project.domain, project.crm?.config, functionInfo);
            if (error) return null;

            toast.loading(`${filteredFunctions.length - (index + 1)} functions remaining`, { id: toastId });
            return data;
          }),
        )
      ).filter(Boolean);
      ...
      const toUpdate = arr.concat(functionsWithCode, project.crm?.functions, 'id').filter((x: any) => x.id);
    ...
    onSuccess: async (data) => {
      ...
      if (activeFn) setActiveFn(data.find((func) => func.id === activeFn.id));
  ...
  const queryParams = useQuery<any>({
    ...
    queryFn: async () => {
      ...
      if (functionParam && project?.crm?.functions?.length) {
        const functionInfo = project.crm.functions.find((func) => func.name === functionParam);
  ...
  const isToCommit = (functionInfo: any) => {
    return time.fixTime(functionInfo.updatedTime) >= lastCommit || time.fixTime(functionInfo.createdTime) >= lastCommit;
  };
    ...
    filterConfig: [
      ...
      },
      {
        ...
        matchFn: (data: any, value: string[]) => {
          ...
          if (value.includes('pending_commits')) return !!commits.filter((c: Commit) => c.status === 'pending')?.length;
        },
      },
    ],
            ...
            <>
              {project?.departments?.id === DEPARMENTS.FTE && (
                <ButtonPush
                  ...
                  functionscommitted={queryCommits.data.pending?.map((commit: Commit) => ({
        ...
        {project?.crm?.functions?.length ? (
          <AppTabContent>
            ...
            {data.length ? (
              <AppTabContentBody>
                {/* List */}
                <div className={cn('flex flex-col gap-10 text-sm', activeFn ? 'col-span-1' : 'col-span-3')}>
                  {functions.map(({ label, items }: any, index: any) => {
                    return (
                      <Collapsible key={index} defaultOpen={true}>
                        <CardContainer>
                          <ListHeaderFunction
                            label={FUNCTIONS_CATEGORIES_OBJ[label as keyof typeof FUNCTIONS_CATEGORIES_OBJ]}
                            length={items.length}
                          />
                ...
                {/* Code */}
                {activeFn && (
                  <CardContainer className="sticky top-4 col-span-2 h-[calc(100vh-40px)] overflow-auto pt-0 w-full">
                    <div className="sticky top-0 z-10 flex w-full items-center justify-between border-b bg-primary-foreground pb-2 pt-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <Description>
                          Last modified date: {format(new Date(activeFn.modified_on), 'dd-MM-yyyy HH:mm:ss')}
                        </Description>
                      </div>
                      {/* Buttons */}
                      <div className="flex items-center gap-1">
                        ...
                        />
                        <ButtonCommitsHistory commits={queryCommits.data?.obj[activeFn.id]} />
                        <ButtonGenerateDoc functionName={activeFn.display_name} code={activeFn?.workflow} />
                        <Link
                          target="_blank"
                        ...
                        <Button variant="ghost" size="icon" onClick={() => setActiveFn(null)}>
                          <X className="size-4" />
                        </Button>
                      </div>
                    </div>
